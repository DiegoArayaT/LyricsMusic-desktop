...
const { getAccessToken, getCurrentSong } = require(path.join(__dirname, '../api/spotify.js'));
...
const LYRICS_APP = (function () {
  ...
  async function loadNewLyrics() {
    ...
    if (!token) {
        ...
        document.querySelector('.lyrics').textContent = "Error de autenticación con Spotify.";
        return;
    }
    ...
    if (!track || track[0] === "" || track[1] === "") {
        ...
        document.querySelector('.lyrics').textContent = "No hay música reproduciéndose.";
        return;
    }
...
document.addEventListener('DOMContentLoaded', async () => {
  const { getAccessToken } = require('../api/spotify.js');
  const token = await getAccessToken();
  if (token) {
    const btn = document.querySelector('button[onclick="authenticateSpotify()"]');
